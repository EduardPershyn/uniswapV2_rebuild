...
import UniswapV2Callee from "../artifacts/contracts/test/UniswapV2Callee.sol/UniswapV2Callee.json";
import FlashBorrower from "../artifacts/contracts/test/FlashBorrower.sol/FlashBorrower.json";
...
describe("UniswapV2Pair", () => {
  ...
  it("swap:gas", async () => {
    ...
    expect(receipt.gasUsed).to.eq(85099);
  });
  ...
  it("flashSwap:swap", async () => {
    const callee = await deployContract(
      wallet,
      UniswapV2Callee,
      [factory.address],
      overrides
    ...
    const loanAmount = expandTo18Decimals(2);
    await token0.transfer(callee.address, loanAmount.mul(1000).div(997)); //supply callee with fee

    await pair.swap(loanAmount, 0, callee.address, "0x1", overrides);

    expect(await token0.balanceOf(pair.address)).to.eq(
      expandTo18Decimals(3).add(
        loanAmount.mul(1000).div(997).sub(loanAmount).add(1)
      )
    );
    expect(await token1.balanceOf(pair.address)).to.eq(expandTo18Decimals(3));
  });

  it("flashSwap:flashLoan", async () => {
    const borrower = await deployContract(
      wallet,
      FlashBorrower,
      [pair.address],
      overrides
    ...
    const loanAmount = expandTo18Decimals(2);
    await token0.transfer(borrower.address, loanAmount.mul(1000).div(997));

    await borrower.flashBorrow(token0.address, loanAmount, overrides);
